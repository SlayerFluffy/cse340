...
/* ****************************************
 * Build select input for add-inventory
 * **************************************** */
Util.getSelect = async function (req, res, next) {
  let data = await invModel.getClassifications()
  let selectOptions = "<select id='classification_id' name-'classification_id'>"
  ...
  data.rows.forEach((row) => {
    ...
    selectOptions += row.classification_id + "'>"
    selectOptions += row.classification_name
